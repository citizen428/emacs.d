...
(defcustom pure-prompt-regexp "^> \\|^[A-Za-z_0-9-]*> \\|^: "
  "Regexp to match prompts in the Pure interpreter.
...
;;   (list "^#!.*" 0 'font-lock-comment-face t)
   ...
   (list "\\_<\\(catch\\|throw\\)\\_>" 0 'font-lock-builtin-face)
   (list
    (concat "\\_<\\("
...
(defvar pure-font-lock-syntactic-keywords
  '(;; #! (shebang) and %< ... %> code sections are treated as comments
    ("^\\(#!\\)" 1 "<") ("^#!.*\\([\n]\\)" 1 ">")
    ("\\(%\\)<" 1 "!" t) ("%\\(>\\)" 1 "!" t)
...
(cond ((not pure-mode-map)
       ...
       (define-key pure-mode-map [C-S-left] 'pure-prev-defun)
       (define-key pure-mode-map [C-S-right] 'pure-next-defun)
       (define-key pure-mode-map [C-S-up]   'pure-backward-defun)
       (define-key pure-mode-map [C-S-down] 'pure-forward-defun)
...
`pure-prev-defun' (\\[pure-prev-defun], <C-S-left>) moves backward to the
beginning of the previous definition at the same or a lower block
level.

`pure-next-defun' (\\[pure-next-defun], <C-S-right>) moves forward to the
beginning of the next definition at the same or a lower block
level.

`pure-backward-defun' (\\[pure-backward-defun], <C-S-up>) moves backward to the
beginning of the previous definition at the same block level,
skipping over definitions in lower levels.

`pure-forward-defun' (\\[pure-forward-defun], <C-S-down>) moves forward to the
...
`pure-send-line' (\\[pure-send-line]), `pure-send-defun' (\\[pure-send-defun]) and
...
(defconst pure-declaration-re   "\\_<\\(const\\|extern\\|infix[lr]?\\|let\\|namespace\\|\\(non\\|out\\|post\\|pre\\)fix\\|private\\|public\\|using\\)\\_>")
...
beginning. Return point if successful, nil otherwise."
  (interactive)
  (unless (pure-at-empty-or-comment-line)
